# Build a Canvas Power App with a SharePoint List and a Power Automate Flow

**Duration:** ~35 minutes
**Level:** Beginner
**Product area:** Microsoft Power Platform (Power Apps, SharePoint, Power Automate)
...
1. Create a **SharePoint list** to store requests.
...
## Part 1 — Create the SharePoint list (~8 min)
...
✅ **Checkpoint:** You have a `Maintenance Requests` list with custom columns.
...
6. **Connect the data:** left rail **Data** → **+ Add data**. Search the
   **SharePoint** connector and select it. Then select an existing connection or
   create a new one. Provide your OneDrive site URL (the one you copied in Part 1)
   and pick the `Maintenance Requests` list.
...
10. Select Fields and map **Subtitle** and **Body** fields:
    ...
    - Body → `ThisItem.Priority.Value` (SharePoint choice columns return a record,
      so use `.Value`)
...
3. Select the Form control (Form1) and set its **OnSuccess** event to call the
   ...
   > `OnSuccess` fires only after the item saves, and `Form1.LastSubmit` holds
...
✅ **Checkpoint:** Submitting the form creates a list item **and** triggers the
...
3. Confirm the new item shows in the gallery.
...
- **`.Value` errors on Priority/Status:** SharePoint choice columns return a
  record — use `.Value` (e.g., `ThisItem.Priority.Value`,
...
- **List not found / connection issues:** Confirm the SharePoint connection uses
  the correct site URL and that you have access to the `Maintenance Requests` list.

---

## What you learned

- Creating a **SharePoint list** with typed and choice columns.
- Building a **responsive Canvas app** (Responsive layout + layout containers)
  that reads (gallery) and writes (edit form) SharePoint data.
...
- Add search/filter to the gallery:
  `Filter(Search('Maintenance Requests', TextInput1.Text, "Title"), Status.Value = "New")`.
